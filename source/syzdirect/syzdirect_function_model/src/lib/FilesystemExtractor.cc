...
Function* FilesystemExtractorPass::findGetTreeFromInitFsCtx(Function* initFsCtx)
{
    ...
    if(!initFsCtx || initFsCtx->isDeclaration()) return res;
    ...
    if(targetOp)
    {
        ...
        if(globalVar != nullptr)
        {
            ...
            if(!getTreeFuncPtr->isNullValue())
            {
                ...
                if(!getTree) {
                    if(auto *CE = dyn_cast<ConstantExpr>(getTreeFuncPtr))
                        getTree = dyn_cast<Function>(CE->stripPointerCasts());
                }
                if(getTree) {
        ...
        if(I->getOpcode() == Instruction::Call)
        {
            ...
            for(Function* callee:Ctx->Callees[callInst])
            {
                if(!callee->isVarArg())
                {
                    ...
                    {
                        if(Function* calledBack = resolveFunction(arg))
                        {
...
void FilesystemExtractorPass::HandleFsTypeStruct(GlobalVariable* globalVar, FilesystemInfoItem* filesystemInfoItem)
{
    ...
    if(!mountFuncPtr->isNullValue())
    {
        ...
        if(!mountFunc) {
            if(auto *CE = dyn_cast<ConstantExpr>(mountFuncPtr))
                mountFunc = dyn_cast<Function>(CE->stripPointerCasts());
        }
        if(mountFunc && mountFunc->getInstructionCount() == 0)
        {
        ...
        if(mountFunc && mountFunc->getInstructionCount() != 0)
        {
            ...
            for(inst_iterator iter = inst_begin(mountFunc); iter != inst_end(mountFunc); iter++)
            {
                ...
                if(I->getOpcode() == Instruction::Call)
                {
                    ...
                    if(callInst->getCalledFunction() != nullptr && callInst->getCalledFunction()->getName() == "mount_bdev")
                    {
                        ...
                        if(!fillSuperFunc) {
                            if(auto *CE = dyn_cast<ConstantExpr>(callInst->getArgOperand(4)))
                                fillSuperFunc = dyn_cast<Function>(CE->stripPointerCasts());
                        }
                        if(fillSuperFunc) outs() << "fill super func: " << fillSuperFunc->getName() << "\n";
                        break;
                    }
                    else if(callInst->getCalledFunction() != nullptr && callInst->getCalledFunction()->getName() == "mount_nodev")
                    {
                        ...
                        if(!fillSuperFunc) {
                            if(auto *CE = dyn_cast<ConstantExpr>(callInst->getArgOperand(3)))
                                fillSuperFunc = dyn_cast<Function>(CE->stripPointerCasts());
                        }
                        if(fillSuperFunc) outs() << "fill super func: " << fillSuperFunc->getName() << "\n";
                        break;
            ...
            if(fillSuperFunc != nullptr)
            {
                ...
                if(fillSuperFunc && fillSuperFunc->getInstructionCount() != 0) {
    ...
    if(!initfsctxFuncPtr->isNullValue())
    {
        ...
        if(!initfsctxFunc) {
            if(auto *CE = dyn_cast<ConstantExpr>(initfsctxFuncPtr))
                initfsctxFunc = dyn_cast<Function>(CE->stripPointerCasts());
        }
        if(initfsctxFunc && initfsctxFunc->getInstructionCount() == 0)
        ...
        if(initfsctxFunc != nullptr)
        {
            ...
            if (getTree == nullptr || getTree->isDeclaration()) {
                return;
            }
            Function* fillSuperFunc = nullptr;
            for (auto iter = inst_begin(getTree); iter != inst_end(getTree); iter++) {
                ...
                if (res != PassGetTreeWrapper.end()) {
                    ...
                    if(!fillSuperFunc) {
                        if(auto *CE = dyn_cast<ConstantExpr>(callInst->getArgOperand((*res).second)))
                            fillSuperFunc = dyn_cast<Function>(CE->stripPointerCasts());
                    }
                    if(fillSuperFunc) {
...
vector<pair<string, Function*>> FilesystemExtractorPass::getHandlerFromFileOperations(GlobalVariable* globalVar)
{
    ...
    {
        Function* writeFunc = resolveFunction(handlerWrite);
        ...
        else
        {
            ...
            {
                writeIterFunc = getFunctionFromModules(writeIterFunc->getName());
            }
            if(writeIterFunc != nullptr)
            {
                res.push_back(make_pair("write", writeIterFunc));
            }
        }
    }
    {
        Function* ioctlFunc = resolveFunction(handlerIoctl);
    ...
    {
        Function* openFunc = resolveFunction(handlerOpen);
        if(openFunc != nullptr)
        {
    ...
    {
        Function* mmapFunc = resolveFunction(handlerMmap);
        if(mmapFunc != nullptr)
        {
...
void FilesystemExtractorPass::ProcessRegisterFilesystem(CallInst* callInst)
{
    ...
    for(auto p:filesystemInfoItem->SyscallHandler){
        ...
        if(!mntFunc || mntFunc->isDeclaration()) continue;
    ...
    for (auto handler : xattrHandlers) {
        ...
        for (auto iterIdx: {0, 1}) {
            ...
            for (auto &handlerItem: xattrHandlerSyscallMap) {
                ...
                Function* handlerFunc = resolveFunction(handlerPtr);
                if (!handlerFunc) {
                    continue;
                }
