    ...
    for (auto item: ModuleMap) {
        ...
        for(int i = 0; i < drm_ioctls_struct->getNumOperands(); i++) {
            ...
            if (auto constantStruct = dyn_cast<ConstantStruct>(item)) {
                ...
                Function* resolvedFunc = dyn_cast<Function>(func_arg);
                if (!resolvedFunc && func_arg) {
                    if (auto *CE = dyn_cast<ConstantExpr>(func_arg))
                        resolvedFunc = dyn_cast<Function>(CE->stripPointerCasts());
                }
                if (resolvedFunc) {
...
map<unsigned, Function*> handleIoctlHandlersGlobalVar(GlobalVariable* ioctlHandlers)
{
    ...
    for(int i = 0; i < ioctlHandlersArray->getNumOperands(); i++)
    {
        ...
        if(cmd == nullptr)
            continue;
        auto handlerVal = ioctlHandler->getOperand(GlobalCtx.StructFieldIdx["ioctl_handler"]["func"]);
        Function* handler = dyn_cast<Function>(handlerVal);
        if(!handler) {
            if(auto *CE = dyn_cast<ConstantExpr>(handlerVal))
                handler = dyn_cast<Function>(CE->stripPointerCasts());
        }
    ...
    for(inst_iterator iter = inst_begin(targetFunction); iter != inst_end(targetFunction); iter++)
    {
        ...
        if(GetElementPtrInst* gepInst = dyn_cast<GetElementPtrInst>(I))
        {
            if(!gepInst->getResultElementType()->isPointerTy())
                continue;
            // With opaque pointers (LLVM 18+), we cannot check if a pointer
            // points to a function type. Check if users load and call through it.
            if(gepInst->getResultElementType()->isPointerTy())
            {
