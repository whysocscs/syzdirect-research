...
void CallGraphPass::findCalleesWithType(CallInst *CI, FuncSet &S) {
	...
	for (Function *F : Ctx->AddressTakenFuncs) {
		...
		for (Function::arg_iterator FI = F->arg_begin(), 
			...
			// With opaque pointers (LLVM 18+), all pointer types are identical,
			// so if both are pointers, they are compatible.
			if (DefinedTy->isPointerTy() && ActualTy->isPointerTy())
				continue;
			if (DefinedTy->isStructTy() && ActualTy->isStructTy() &&
					cast<StructType>(DefinedTy)->hasName() && cast<StructType>(ActualTy)->hasName() &&
...
bool CallGraphPass::typeConfineInStore(StoreInst *SI) {
	...
	// With opaque pointers (LLVM 18+), the element type of the store
	// destination is the type of the value being stored.
	...
	if (isa<ConstantPointerNull>(VO))
		return false;
	// With opaque pointers, we cannot get the element type from a pointer type.
	// Conservative: skip this case.
	if (!VO->getType()->isPointerTy())
		return false;

	// With opaque pointers, we cannot determine the pointed-to type from
	// the pointer type alone. Try to find it via nextLayerBaseType instead.
	Type *EVTy = nullptr;
...
bool CallGraphPass::typeConfineInCast(CastInst *CastI) {
	...
	if (!FromTy->isPointerTy() || !ToTy->isPointerTy())
		return false;
	// With opaque pointers (LLVM 18+), we cannot get element types from
	// pointer types. All pointer casts are no-ops, so skip this analysis.
	// This is conservative (may miss some type confinement).
