...
static Function* resolveFunction(Value* V) {
    ...
    if (auto *CE = dyn_cast<ConstantExpr>(V))
        return dyn_cast<Function>(CE->stripPointerCasts());
    return nullptr;
}
...
void NetworkInterfaceExtractorPass::getProtoOpsFromCreateFunction(Function* F, ConstantStruct** protoOps, set<Function*>&visited, int depth)
{
    ...
    for(inst_iterator iter = inst_begin(F); iter != inst_end(F); iter++)
    {
        ...
        else if(I->getOpcode() == Instruction::GetElementPtr)
        {
            ...
            if(!gepInst->getSourceElementType()->isStructTy() || !cast<StructType>(gepInst->getSourceElementType())->hasName())
            {
                continue;
            }
            if(gepInst->getSourceElementType()->getStructName() == "struct.socket")
            {
                ...
                // With opaque pointers (LLVM 18+), we cannot get the
                // element type of a pointer. Accept all pointer fields
                // from the named source struct.
                {
                    for(User* user:gepInst->users())
                    {
                        if(StoreInst* storeInst = dyn_cast<StoreInst>(user))
                        {
                            ...
                            if(globalVar != nullptr)
                            {
                                ...
                                if(ops == nullptr && globalVar->hasName() && Ctx->GlobalStructMap.count(globalVar->getName().str()))
                                    ops = getStructValue(Ctx->GlobalStructMap[globalVar->getName().str()]);
                                if(ops)
                                {
                            ...
                            for(inst_iterator iter = inst_begin(targetFunc); iter != inst_end(targetFunc); iter++)
                            {
                                ...
                                if(CallInst* callInst = dyn_cast<CallInst>(I))
                                {
                                    ...
                                    if(callInst->getCalledFunction()->getName() == "netlink_rcv_skb")
                                    {
                                        ...
                                        if(!targetFunc) {
                                            if(auto *CE = dyn_cast<ConstantExpr>(callInst->getArgOperand(1)))
                                                targetFunc = dyn_cast<Function>(CE->stripPointerCasts());
                                        }
                                        return targetFunc;
                                    }
                                }
                    ...
                    for(User* user:F->users())
                    {
                        if(CallInst* callInst = dyn_cast<CallInst>(user))
                        {
                           ...
                           if(constantInt1 && constantInt2)
                           {
                               ...
                               if(!doitFunc && !isa<ConstantPointerNull>(op3)) {
                                   if(auto *CE = dyn_cast<ConstantExpr>(op3))
                                       doitFunc = dyn_cast<Function>(CE->stripPointerCasts());
                               }
                               Function* dumpitFunc = dyn_cast<Function>(op4);
                               if(!dumpitFunc && !isa<ConstantPointerNull>(op4)) {
                                   if(auto *CE = dyn_cast<ConstantExpr>(op4))
                                       dumpitFunc = dyn_cast<Function>(CE->stripPointerCasts());
                               }
...
vector<tuple<unsigned, Function*, Function*>> NetworkInterfaceExtractorPass::getGenlFamilyHandlers(ConstantStruct* familyStruct)
{
    ...
    if(opsVar != nullptr)
    {
        ...
        for(int i =  0; i < opsArray->getNumOperands(); i++)
        {
            ...
            if (!opStruct) continue;
            auto cmd = opStruct->getOperand(Ctx->StructFieldIdx["genl_ops"]["cmd"]);
            auto cmdVal = getIntValue(cmd);
            auto doitVal = opStruct->getOperand(Ctx->StructFieldIdx["genl_ops"]["doit"]);
            Function* doitFunc = dyn_cast<Function>(doitVal);
            if(!doitFunc) if(auto *CE = dyn_cast<ConstantExpr>(doitVal)) doitFunc = dyn_cast<Function>(CE->stripPointerCasts());
            auto dumpitVal = opStruct->getOperand(Ctx->StructFieldIdx["genl_ops"]["dumpit"]);
            Function* dumpitFunc = dyn_cast<Function>(dumpitVal);
            if(!dumpitFunc) if(auto *CE = dyn_cast<ConstantExpr>(dumpitVal)) dumpitFunc = dyn_cast<Function>(CE->stripPointerCasts());
    ...
    if(smallOpsVar != nullptr)
    {
        ...
        for(int i =  0; i < smallOpsArray->getNumOperands(); i++)
        {
            ...
            if (!smallOpStruct) continue;
            auto cmd = smallOpStruct->getOperand(Ctx->StructFieldIdx["genl_small_ops"]["cmd"]);
            auto cmdVal = getIntValue(cmd);
            auto smallDoitVal = smallOpStruct->getOperand(Ctx->StructFieldIdx["genl_small_ops"]["doit"]);
            Function* doitFunc = dyn_cast<Function>(smallDoitVal);
            if(!doitFunc) if(auto *CE = dyn_cast<ConstantExpr>(smallDoitVal)) doitFunc = dyn_cast<Function>(CE->stripPointerCasts());
            auto smallDumpitVal = smallOpStruct->getOperand(Ctx->StructFieldIdx["genl_small_ops"]["dumpit"]);
            Function* dumpitFunc = dyn_cast<Function>(smallDumpitVal);
            if(!dumpitFunc) if(auto *CE = dyn_cast<ConstantExpr>(smallDumpitVal)) dumpitFunc = dyn_cast<Function>(CE->stripPointerCasts());
            res.push_back(tuple<unsigned, Function*, Function*>(cmdVal, doitFunc, dumpitFunc));
        }
    }
    return res;
...
GenlFamilyInfo* NetworkInterfaceExtractorPass::getGenlFamilyInfo(ConstantStruct* familyStruct)
{
    ...
    if(policyVar != nullptr)
    {
        ...
        if(policyStructVal == nullptr && policyVar->hasName() && Ctx->GlobalStructMap.count(policyVar->getName().str()))
            policyStructVal = getStructValue(Ctx->GlobalStructMap[policyVar->getName().str()]);
        if(policyStructVal != nullptr)
        {
            ConstantStruct* policyStruct = dyn_cast<ConstantStruct>(policyStructVal);
            genlFamilyInfo->policy = policyStruct; // may be nullptr, which is handled
        }
        else
        {
                    ...
                    for(User* user:F->users())
                    {
                        if(CallInst* callInst = dyn_cast<CallInst>(user))
                        {
                            ...
                            if(globalVar != nullptr)
                            {
                                ...
                                if(familyStructVal == nullptr && globalVar->hasName() && Ctx->GlobalStructMap.count(globalVar->getName().str()))
                                    familyStructVal = getStructValue(Ctx->GlobalStructMap[globalVar->getName().str()]);
                                if(familyStructVal)
                                {
                                    ConstantStruct* familyStruct = dyn_cast<ConstantStruct>(familyStructVal);
                                    if (!familyStruct) continue;
...
vector<tuple<unsigned, Function*, Function*>> NetworkInterfaceExtractorPass::getXfrmHandlers()
{
    ...
    for(int i = 0; i < xfrmDispathArray->getNumOperands(); i++)
    {
        ...
        if(xfrmLinkStruct == nullptr)
            continue;
        auto xfrmDoitVal = xfrmLinkStruct->getOperand(Ctx->StructFieldIdx["xfrm_link"]["doit"]);
        Function* doitFunc = dyn_cast<Function>(xfrmDoitVal);
        if(!doitFunc) if(auto *CE = dyn_cast<ConstantExpr>(xfrmDoitVal)) doitFunc = dyn_cast<Function>(CE->stripPointerCasts());
        auto xfrmDumpitVal = xfrmLinkStruct->getOperand(Ctx->StructFieldIdx["xfrm_link"]["dumpit"]);
        Function* dumpitFunc = dyn_cast<Function>(xfrmDumpitVal);
        if(!dumpitFunc) if(auto *CE = dyn_cast<ConstantExpr>(xfrmDumpitVal)) dumpitFunc = dyn_cast<Function>(CE->stripPointerCasts());
...
map<unsigned, Function*> NetworkInterfaceExtractorPass::getNfSubsysHandlers(ConstantStruct* nfSubsysStruct)
{
    ...
    if(cbVar != nullptr)
    {
        ...
        for(int i = 0; i < cbArray->getNumOperands(); i++)
        {
            ...
            if(cbStruct == nullptr)
                continue;
            auto cbCallVal = cbStruct->getOperand(Ctx->StructFieldIdx["nfnl_callback"]["call"]);
            Function* handler = dyn_cast<Function>(cbCallVal);
            if(!handler) if(auto *CE = dyn_cast<ConstantExpr>(cbCallVal)) handler = dyn_cast<Function>(CE->stripPointerCasts());
            res[i] = handler;
        }
    }
    return res;
                            ...
                            if(constantInt1)
                            {
                                ...
                                if(cb_table != nullptr)
                                {
                                    ...
                                    for(int i = 0; i < cbArray->getNumOperands(); i++)
                                    {
                                        ...
                                        if(cbStruct == nullptr)
                                            continue;
                                        auto rdmaDoitVal = cbStruct->getOperand(Ctx->StructFieldIdx["rdma_nl_cbs"]["doit"]);
                                        Function* doit = dyn_cast<Function>(rdmaDoitVal);
                                        if(!doit) if(auto *CE = dyn_cast<ConstantExpr>(rdmaDoitVal)) doit = dyn_cast<Function>(CE->stripPointerCasts());
                                        auto rdmaDumpVal = cbStruct->getOperand(Ctx->StructFieldIdx["rdma_nl_cbs"]["dump"]);
                                        Function* dump = dyn_cast<Function>(rdmaDumpVal);
                                        if(!dump) if(auto *CE = dyn_cast<ConstantExpr>(rdmaDumpVal)) dump = dyn_cast<Function>(CE->stripPointerCasts());
                    ...
                    for(User* user:F->users())
                    {
                        if(CallInst* callInst = dyn_cast<CallInst>(user))
                        {
                            ...
                            if(globalVar != nullptr)
                            {
                                ...
                                if(subSysStructVal == nullptr && globalVar->hasName() && Ctx->GlobalStructMap.count(globalVar->getName().str()))
                                    subSysStructVal = getStructValue(Ctx->GlobalStructMap[globalVar->getName().str()]);
                                if(subSysStructVal)
                                {
                                    ConstantStruct* subsysStruct = dyn_cast<ConstantStruct>(subSysStructVal);
                                    if (!subsysStruct) continue;
...
set<Value*>* NetworkInterfaceExtractorPass::GetAliasOfStructType(Value* value, string structName) {
    ...
    for (auto A : *aliasSet) {
        // With opaque pointers (LLVM 18+), we cannot get element type
        // from a PointerType. Try to find the struct type from the value.
        if (A->getType()->isPointerTy()) {
            Type *ElemType = nullptr;
            if (auto *AI = dyn_cast<AllocaInst>(A))
                ElemType = AI->getAllocatedType();
            else if (auto *GEP = dyn_cast<GetElementPtrInst>(A))
