...
map<std::string, Function*> DeviceExtractorPass::ProcessFileOperations(Value* handlerStruct) {
    ...
    if (auto constantStruct = dyn_cast<ConstantStruct>(handlerStruct)) {
        ...
        if (auto *writeFunc = resolveFunction(handlerWrite)) {
        ...
        if (auto *ioctlFunc = resolveFunction(handlerIoctl)) {
            result["ioctl"] = ioctlFunc;
            GlobalCtx.FunctionArgMap[ioctlFunc->getName().str()] = {1, 2, 4};
        }
        if (auto *openFunc = resolveFunction(handlerOpen)) {
            result["open"] = openFunc;
            GlobalCtx.FunctionArgMap[openFunc->getName().str()] = {0, 1};
        }
        if (auto *mmapFunc = resolveFunction(handlerMmap)) {
            result["mmap"] = mmapFunc;
...
map<std::string, Function*> DeviceExtractorPass::ProcessPosixClockOperations(Value* handlerStruct) {
    ...
    if (auto constantStruct = dyn_cast<ConstantStruct>(handlerStruct)) {
        ...
        if (auto *openFunc = resolveFunction(handlerOpen)) {
            result["open"] = openFunc;
            GlobalCtx.FunctionArgMap[openFunc->getName().str()] = {1, 2};
        }
        if (auto *readFunc = resolveFunction(handlerRead)) {
            result["read"] = readFunc;
            GlobalCtx.FunctionArgMap[readFunc->getName().str()] = {1, 0, 2, 4};
        }
    }
...
map<std::string, Function*> DeviceExtractorPass::ProcessBlockDeviceOperations(Value* handlerStruct) {
    ...
    if (auto constantStruct = dyn_cast<ConstantStruct>(handlerStruct)) {
        ...
        if (auto *ioctlFunc = resolveFunction(handlerIoctl)) {
            result["ioctl"] = ioctlFunc;
            GlobalCtx.FunctionArgMap[ioctlFunc->getName().str()] = {1, 0, 2, 4};
        }
        if (auto *openFunc = resolveFunction(handlerOpen)) {
            result["open"] = openFunc;
            GlobalCtx.FunctionArgMap[openFunc->getName().str()] = {1, 2};
...
map<string, Function*> DeviceExtractorPass::ProcessTtyOperations(Value* handlerStruct) {
    ...
    if (auto constantStruct = dyn_cast<ConstantStruct>(handlerStruct)) {
        ...
        if (auto *ioctlFunc = resolveFunction(handlerIoctl)) {
            ...
            if (auto *compatIoctlFunc = resolveFunction(handlerIoctl)) {
                result["ioctl"] = compatIoctlFunc;
                GlobalCtx.FunctionArgMap[compatIoctlFunc->getName().str()] = {1, 2, 4};
            }
        }
        if (auto *openFunc = resolveFunction(handlerOpen)) {
            result["open"] = openFunc;
        ...
        if (auto *writeFunc = resolveFunction(handlerWrite)) {
            result["write"] = writeFunc;
            GlobalCtx.FunctionArgMap[writeFunc->getName().str()] = {1, 2, 4};
        }
    }
...
set<Value*>* DeviceExtractorPass::GetAliasOfStructType(Value* value, string structName) {
    ...
    for (auto A : *aliasSet) {
        ...
        // from a PointerType. Instead, try to find the struct type from
        // the value's allocation or GEP source.
        if (A->getType()->isPointerTy()) {
            // Try to get struct type from alloca or GEP source
            ...
            if (constStruct && constStruct->getType()->getName() == "struct.posix_clock_operations") {
                ...
                for (auto i = 0; i < constArray->getNumOperands(); i++) {
                    ...
                    if (constStruct && constStruct->getType()->getName() == "struct.cftype") {
                        ...
                        Function* writeFunc = resolveFunction(writeHandler);
                        if (!writeFunc) {
                            writeHandler = constStruct->getOperand(Ctx->StructFieldIdx["cftype"]["write_u64"]);
                            writeFunc = resolveFunction(writeHandler);
                        }
                        if (writeFunc) {
                        ...
                        if (auto *procFunc = resolveFunction(writeHandler)) {
                            deviceInfoItem->SyscallHandler["write"] = procFunc;
                            Ctx->SubsystemInfo.push_back(deviceInfoItem);
                        }
                    } else {
                        break;
