		...
		for (inst_iterator ii = inst_begin(Func), e = inst_end(Func);
			...
			for(int i = 0; i < T; i++) {
				...
				if (!VI->hasName())
					continue;
				Type *VT = VI->getType();
				// With opaque pointers (LLVM 18+), we cannot peel
				// pointer types. Try to get the type from the value.
				if (VT->isPointerTy()) {
					if (auto *AI = dyn_cast<AllocaInst>(VI))
						VT = AI->getAllocatedType();
					else if (auto *GEP = dyn_cast<GetElementPtrInst>(VI))
						VT = GEP->getResultElementType();
					else
						continue;
				}
				if (!VT)
