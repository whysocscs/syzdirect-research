...
string Generator::ExtractType(Type* Ty) {
    if (Ty->isIntegerTy()) {
        ...
        // With opaque pointers (LLVM 18+), we cannot get the element type.
        // Default to a generic pointer description.
    ...
    } else if (Ty->isStructTy() && cast<StructType>(Ty)->hasName()) {
...
void Generator::ExtractStructure(string structName, StructType* structType) {
    ...
    for (int i = 0; i < structType->getStructNumElements(); i++) {
        ...
        if (Ty->isPointerTy()) {
            // With opaque pointers (LLVM 18+), we cannot determine
            // the pointed-to type from PointerType. Skip recursive
            // struct extraction for pointer fields.
        }
...
void BasicBlockConstraint::SetConstraintToSyscall(SyscallInstance* instance) {
    ...
    // prev
    SmallVector<BasicBlock *, 16> RevBBs(llvm::make_pointer_range(*func));
    std::reverse(RevBBs.begin(), RevBBs.end());
    for (auto *BBPtr : RevBBs) {
        auto &BB = *BBPtr;
    ...
    for (auto item: dstValueSet) {
        ...
        if (item->getType()->isPointerTy()) {
            // With opaque pointers (LLVM 18+), try to get the struct
            // type from the value source (alloca or GEP).
            Type *PETy = nullptr;
            if (auto *AI = dyn_cast<AllocaInst>(item))
                PETy = AI->getAllocatedType();
            else if (auto *GEP = dyn_cast<GetElementPtrInst>(item))
                PETy = GEP->getResultElementType();
            if (PETy && PETy->isStructTy()) {
    ...
    if (structTys.empty()) {
        ...
        for (auto structTy: structTys) {
            if (!structTy->isStructTy() || !cast<StructType>(structTy)->hasName()) continue;
