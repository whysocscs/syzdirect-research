		...
		for(auto BB:visit){
			for (BasicBlock::iterator I = BB->begin(),
				...
				if (LoadInst *LI = dyn_cast<LoadInst>(&*I)) {
					// With opaque pointers we cannot peel through pointer
					// layers to discover the struct type.  Instead, check
					// whether the loaded value is a pointer (would have
					// been nlattr**) and use name-based heuristics.
					auto oriType = LI->getType();
					if (!oriType->isPointerTy())
						continue;
					// Heuristic: check if the load operand name or
					// debug info references nlattr.
					bool isNlattr = false;
					if (LI->getPointerOperand()->hasName()) {
						StringRef name = LI->getPointerOperand()->getName();
						if (name.contains("nlattr") || name.contains("nla"))
							isNlattr = true;
					}
					if (LI->hasName()) {
					...
					// Also check the alloca/GEP source element type
					if (auto *AI = dyn_cast<AllocaInst>(LI->getPointerOperand())) {
						Type *allocTy = AI->getAllocatedType();
						while (allocTy->isPointerTy()) {
...
//  what if the argument itself is nlattr**?
		if (!ArrayRoot){
			for(auto& arg: F->args()){
				...
				// With opaque pointers, argument types for nlattr**
				// are just 'ptr'.  Use name-based heuristic.
				if (!argType->isPointerTy())
					continue;
				bool isNlattr = false;
				if (arg.hasName()) {
					StringRef name = arg.getName();
					if (name.contains("nlattr") || name.contains("nla") || name.contains("attrs"))
						isNlattr = true;
				}
				if (!isNlattr)
					continue;
...
map<string, BasicBlock*> NewParseDataset(){
	...
	// Build reverse map: functionName -> xidx
	map<string, int> functionName2index;
	for (auto &kv : index2functionName) {
		functionName2index[kv.second] = kv.first;
	}

	// Find target functions by name and use their entry blocks.
	// This avoids depending on kcov_mark_block argument values which
	// are distance values (-1 when no .dist files), not target indices.
	for (auto p : GlobalCtx.Modules) {
		...
		for (auto mi = M->begin(), ei = M->end(); mi != ei; mi++) {
			...
			if (functionName2index.count(fname) == 0)
				continue;
			int xidx = functionName2index[fname];
			if (TargetIndex != -1 && xidx != TargetIndex)
				continue;
			// Use the first kcov_mark_block block if available,
			// otherwise use the entry block.
			BasicBlock *targetBB = &F->getEntryBlock();
			for (auto &inst : instructions(F)) {
				if (CallInst *CI = dyn_cast<CallInst>(&inst)) {
					if (getCalledFuncName(CI) == "kcov_mark_block" ||
					    getCalledFuncName(CI) == "__sanitizer_cov_trace_pc") {
						targetBB = CI->getParent();
						break;
					}
				}
			}
			INFO("Found target function: " << fname << " xidx=" << xidx << "\n");
			Dataset[to_string(xidx)] = targetBB;
		}
	}
