...
void CallGraphPass::findCalleesWithType(CallInst *CI, FuncSet &S) {
	...
	for (Function *F : Ctx->AddressTakenFuncs) {
		...
		for (Function::arg_iterator FI = F->arg_begin(), 
			...
			// With opaque pointers, we cannot peel pointer layers.
			// If both are pointers, treat them as compatible.
...
bool CallGraphPass::typeConfineInStore(StoreInst *SI) {
	...
	// With opaque pointers, the pointer element type equals the stored
	// value type, so use VTy directly for both.
	Type *VTy = VO->getType();
	Type *EPTy = VTy;
	...
	// With opaque pointers, we can't get the element type from PointerType.
	// Conservatively treat as non-composite pointer.
	Type *EVTy = nullptr;
...
bool CallGraphPass::typeConfineInCast(CastInst *CastI) {
	...
	// With opaque pointers, we cannot retrieve the element types of
	// pointer-to-pointer casts, so skip this refinement.
	return false;
}
